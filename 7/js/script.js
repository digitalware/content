...
// ──────────────────────────────────────────────
const gradeConfig = {
    '좋음':    { priority: 1, className: 'grade-good',     desc: '공기가 맑고 안정적입니다' },
    '보통':    { priority: 2, className: 'grade-moderate', desc: '대체로 쾌적한 대기 상태입니다' },
    '나쁨':    { priority: 3, className: 'grade-bad',      desc: '외부 활동 시 가벼운 주의가 필요합니다' },
    '매우나쁨': { priority: 4, className: 'grade-very-bad', desc: '대기 질이 좋지 않습니다. 실내 환경을 권장합니다' }
};
...
function displayAirQualityData(data) {
    ...
    airDescEl.textContent = (isPm10Valid || isPm25Valid) ? worst.desc : '측정 중입니다';
    airDescEl.className = `air-desc ${(isPm10Valid || isPm25Valid) ? worst.className : ''}`;
...
function saveAirQualityDataToCache(data) {
    try {
        safeStorageSetItem(AIR_QUALITY_CACHE_KEY, JSON.stringify(data));
...
function saveWeatherDataToCache(currentData, forecastData) {
    try {
        safeStorageSetItem(WEATHER_CACHE_KEY, JSON.stringify({ current: currentData, forecast: forecastData }));
