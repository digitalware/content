...
// applySavedLocation()은 location-loader.js에서 자동 실행됨

// 페이지 준비 후 제목과 콘솔 업데이트
function updatePageWithLocation() {
    const h1Element = document.querySelector('header h1');
    if (h1Element && CONFIG.AIR_QUALITY_STATION) {
        h1Element.textContent = CONFIG.AIR_QUALITY_STATION;
    }
    
    console.log('날씨 페이지 설정:', {
        위치: CONFIG.AIR_QUALITY_STATION,
        위도: CONFIG.LAT,
        경도: CONFIG.LON
    });
}
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', updatePageWithLocation);
} else {
    updatePageWithLocation();
}
...
function displayAirQualityData(data) {
    ...
    // 수치 표시
    document.getElementById('pm10').innerHTML = isPm10Valid ? `${pm10Value} <em>㎍/㎥</em>` : '측정 중';
    document.getElementById('pm25').innerHTML = isPm25Valid ? `${pm25Value} <em>㎍/㎥</em>` : '측정 중';

    // setAirQualityUI 호출 - 색상과 이미지 적용
    setAirQualityUI(pm10Value, pm25Value);
}
    function getAirState(value,type){
...
function setAirQualityUI(pm10Value,pm25Value){
    ...
    const pm10Grade=getAirState(pm10Value,'pm10');
    const pm25Grade=getAirState(pm25Value,'pm25');

    const worstGrade=
        worstGradePriority[pm10Grade] >= worstGradePriority[pm25Grade]
        ? pm10Grade
        : pm25Grade;

    const airFigure=document.getElementById('air-quality-icon-wrap');
    const airIcon=document.getElementById('air-quality-icon');

    airFigure.className='';
    airFigure.classList.add(worstGrade === 'good' ? 'air-good' :
                            worstGrade === 'moderate' ? 'air-moderate' :
                            worstGrade === 'bad' ? 'air-bad' :
                            'air-very-bad');

    airIcon.src=`images/${worstGrade}.png`;

    const pm10GradeEl=document.getElementById('pm10-grade');
    const pm25GradeEl=document.getElementById('pm25-grade');

    pm10GradeEl.classList.remove('pm-good', 'pm-moderate', 'pm-bad', 'pm-very-bad');
    pm25GradeEl.classList.remove('pm-good', 'pm-moderate', 'pm-bad', 'pm-very-bad');

    pm10GradeEl.classList.add(
        pm10Grade === 'good' ? 'pm-good' :
        pm10Grade === 'moderate' ? 'pm-moderate' :
        pm10Grade === 'bad' ? 'pm-bad' :
        'pm-very-bad'
    );

    pm25GradeEl.classList.add(
        pm25Grade === 'good' ? 'pm-good' :
        pm25Grade === 'moderate' ? 'pm-moderate' :
        pm25Grade === 'bad' ? 'pm-bad' :
        'pm-very-bad'
    );

    pm10GradeEl.textContent=
        pm10Grade === 'good' ? 'Good' :
        pm10Grade === 'moderate' ? 'Moderate' :
        pm10Grade === 'bad' ? 'Bad' :
        'Very Bad';

    pm25GradeEl.textContent=
        pm25Grade === 'good' ? 'Good' :
        pm25Grade === 'moderate' ? 'Moderate' :
        pm25Grade === 'bad' ? 'Bad' :
        'Very Bad';
}
...
function hasMatchingLocation(data) {
    if (!data || typeof data !== 'object' || !data.location) return true;
    const location = data.location;
    return location.lat === CONFIG.LAT && location.lon === CONFIG.LON && location.sido === CONFIG.AIR_QUALITY_SIDO && location.station === CONFIG.AIR_QUALITY_STATION;
}
...
function saveAirQualityDataToCache(data) {
    try {
        localStorage.setItem(AIR_QUALITY_CACHE_KEY, JSON.stringify({ ...data, location: getLocationSnapshot() }));
...
function saveWeatherDataToCache(currentData, forecastData) {
    try {
        localStorage.setItem(WEATHER_CACHE_KEY, JSON.stringify({ current: currentData, forecast: forecastData, location: getLocationSnapshot() }));
