let CONFIG = {
    ...
    function displayAirQualityData(data) {
        ...
        document.getElementById('pm10-grade').textContent = isPm10Valid ? pm10Info.grade : '-';
        ...
        document.getElementById('pm25-grade').textContent = isPm25Valid ? pm25Info.grade : '-';
        document.getElementById('pm25-desc').textContent = isPm25Valid ? pm25Info.desc : '측정 중입니다';
...
function setBackgroundVideo(weatherMain, iconCode) {
    ...
    // 날씨 조건을 먼저 판단하고, 밤인 경우 별도 처리
    switch (weatherMain) {
        case "Clear":
            src = isNight ? "images/weather/vid/night.mp4" : "images/weather/vid/clear.mp4";
            break;

        case "Clouds":
            src = isNight ? "images/weather/vid/night.mp4" : "images/weather/vid/clouds.mp4";
            break;

        case "Rain":
        case "Drizzle":
        case "Thunderstorm":
            // 밤낮 구분 없이 비/천둥번개 영상 사용 (더 중요한 날씨 조건)
            src = "images/weather/vid/rain.mp4";
            break;

        case "Snow":
            // 밤낮 구분 없이 눈 영상 사용 (더 중요한 날씨 조건)
    ...
    console.log(`[배경영상 변경] 날씨: ${weatherMain}, 아이콘: ${iconCode}, 선택영상: ${src}`);
...
function changeVideo(src, loop) {
    ...
    // 같은 영상인지 비교 (절대경로 vs 상대경로 모두 처리)
    const videoFileName = src.split('/').pop(); // "clouds.mp4" 추출
    const currentFileName = bgVideo.src.split('/').pop(); // 현재 파일명 추출
    
    if (videoFileName === currentFileName) return;
    ...
    setTimeout(() => {
        ...
        // 영상 로딩 실패 처리
        const handleError = () => {
            console.error('비디오 로딩 실패:', bgVideo.error);
            bgVideo.style.opacity = 1;
        };

        // 기존 리스너 제거
        bgVideo.removeEventListener('loadeddata', bgVideo._loadedDataHandler);
        bgVideo.removeEventListener('error', bgVideo._errorHandler);

        // 새 리스너 등록 및 참조 저장
        bgVideo._loadedDataHandler = handleLoadedData;
        bgVideo._errorHandler = handleError;
